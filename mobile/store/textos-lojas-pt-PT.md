# Textos das lojas — ReparAuto (Português de Portugal)

Conteúdo pronto a colar nas fichas da **App Store** e da **Google Play**.
Respeita os limites de caracteres de cada campo. Revê antes de submeter.

---

## Comum

- **Nome da app:** ReparAuto
- **Categoria principal:** Compras (Shopping)
- **Categoria secundária (App Store):** Estilo de vida
- **Idioma principal:** Português (Portugal)
- **Classificação etária:** 4+ (App Store) / 3+ (Google Play)
- **URL de marketing:** https://reparauto.pt
- **URL de política de privacidade:** https://reparauto.pt/privacidade
- **URL de suporte:** https://reparauto.pt/seguranca
- **Email de suporte:** suporte@reparauto.pt

---

## App Store (iOS)

### Nome (máx. 30 caracteres)
```
ReparAuto
```

### Subtítulo (máx. 30 caracteres)
```
Carros, peças e oficinas
```

### Texto promocional (máx. 170 caracteres)
```
Compre e venda carros usados e peças, encontre oficinas perto de si e fale
diretamente com os vendedores. Tudo num só lugar.
```

### Descrição (máx. 4000 caracteres)
```
A ReparAuto é o marketplace português para quem compra, vende e repara
automóveis. Encontre o seu próximo carro usado, peças ao melhor preço e oficinas
de confiança perto de si — tudo numa app rápida e simples.

COMPRAR E VENDER CARROS
• Milhares de viaturas usadas, com fotos, preço, quilómetros e detalhes.
• Pesquise por marca, modelo ou localidade e filtre por preço.
• Anuncie o seu carro em minutos: tire fotos, defina o preço e publique.

PEÇAS USADAS
• Compre, venda ou procure peças para qualquer marca e modelo.
• Anúncios de venda, desmonte e procura.

OFICINAS PERTO DE SI
• Diretório de oficinas com especialidades, contactos e avaliações.
• Veja as oficinas no mapa e encontre as mais próximas de si.
• Registe a sua oficina e receba novos clientes.

PROCURA UM CARRO?
• Publique o que procura e deixe os vendedores virem ter consigo.

FALE DIRETAMENTE
• Converse por chat dentro da app, com notificações de novas mensagens.
• Contacte vendedores e oficinas por telefone ou WhatsApp.
• Guarde os seus anúncios favoritos para ver mais tarde.

PARA PROFISSIONAIS E PARTICULARES
• Conta gratuita para particulares e profissionais.
• Faça a gestão dos seus anúncios e acompanhe o estado de cada um.

Pode navegar livremente sem criar conta. O início de sessão só é pedido para
guardar favoritos, anunciar ou contactar.

Descarregue a ReparAuto e junte-se ao maior mercado automóvel português.
```

### Palavras-chave (máx. 100 caracteres, separadas por vírgula)
```
carros usados,automóveis,peças,oficinas,stand,viaturas,comprar carro,vender carro,auto,reparação
```

### Notas de versão — "Novidades" (máx. 4000 caracteres)
```
• Listagens de carros, peças e oficinas em tempo real, com pesquisa e filtros.
• Publique anúncios de carros, peças, oficinas e procuras, com fotos.
• Chat em tempo real e notificações de novas mensagens.
• Mapa de oficinas e avaliações.
• Favoritos, perfil e gestão dos seus anúncios.
• Início de sessão com email, Google e Apple.
```

### Notas para o revisor (App Review)
```
A app permite navegar como convidado, sem conta. O início de sessão só é pedido
para ações (favoritar, anunciar, gerir perfil).

Conta de teste:
  Email: revisor@reparauto.pt
  Palavra-passe: (fornecer)

Eliminação de conta: disponível em Perfil → Eliminar conta.
Sign in with Apple disponível no ecrã de início de sessão (iOS).
```

---

## Google Play (Android)

### Nome da app (máx. 30 caracteres)
```
ReparAuto
```

### Descrição curta (máx. 80 caracteres)
```
Carros usados, peças e oficinas. Compre, venda e contacte num só lugar.
```

### Descrição completa (máx. 4000 caracteres)
```
A ReparAuto é o marketplace português para quem compra, vende e repara
automóveis. Encontre o seu próximo carro usado, peças ao melhor preço e oficinas
de confiança perto de si — tudo numa app rápida e simples.

COMPRAR E VENDER CARROS
• Milhares de viaturas usadas, com fotos, preço, quilómetros e detalhes.
• Pesquise por marca, modelo ou localidade e filtre por preço.
• Anuncie o seu carro em minutos: tire fotos, defina o preço e publique.

PEÇAS USADAS
• Compre, venda ou procure peças para qualquer marca e modelo.
• Anúncios de venda, desmonte e procura.

OFICINAS PERTO DE SI
• Diretório de oficinas com especialidades, contactos e avaliações.
• Veja as oficinas no mapa e encontre as mais próximas de si.
• Registe a sua oficina e receba novos clientes.

PROCURA UM CARRO?
• Publique o que procura e deixe os vendedores virem ter consigo.

FALE DIRETAMENTE
• Converse por chat dentro da app, com notificações de novas mensagens.
• Contacte vendedores e oficinas por telefone ou WhatsApp.
• Guarde os seus anúncios favoritos para ver mais tarde.

PARA PROFISSIONAIS E PARTICULARES
• Conta gratuita para particulares e profissionais.
• Faça a gestão dos seus anúncios e acompanhe o estado de cada um.

Pode navegar livremente sem criar conta. O início de sessão só é pedido para
guardar favoritos, anunciar ou contactar.

Descarregue a ReparAuto e junte-se ao maior mercado automóvel português.
```

### Notas da versão (máx. 500 caracteres)
```
Primeira versão: listagens de carros, peças e oficinas com pesquisa e filtros;
publicação de anúncios e procuras com fotos; chat e notificações; mapa de
oficinas e avaliações; favoritos; perfil e gestão de anúncios; início de sessão
com email, Google e Apple.
```

### Data safety (resumo a declarar no formulário)
```
Dados recolhidos e associados ao utilizador:
• Informações de contacto: email, telefone.
• Conteúdo do utilizador: fotos dos anúncios e mensagens de chat.
• Localização aproximada: apenas em uso, para mostrar oficinas próximas no mapa.
• Identificadores: ID de utilizador e token de notificações (Firebase).

• Os dados são encriptados em trânsito.
• O utilizador pode pedir a eliminação da conta e dados na app
  (Perfil → Eliminar conta).
• Não há partilha de dados com terceiros para publicidade.
• Não é feito tracking entre apps.
```

---

## Notas

- Substituir os URLs e o email de exemplo pelos reais antes de submeter.
- O nome legal/empresa pode ser pedido nas duas consolas (preencher com a
  entidade responsável pela ReparAuto).
- Screenshots: gerar a partir da app em execução (Início, Detalhe do carro,
  Peças, Oficinas, Anunciar, Favoritos).
