// FIPE vehicle catalog for the Brazilian market (plan 20). Brands/models come
// from the public Parallelum FIPE API (BrasilAPI's FIPE proxy currently 403s)
// and are cached in localStorage so the Anunciar flow doesn't hit the API —
// and its rate limit — on every load.
...
export async function fetchFipeBrands(tipo?: TipoVeiculo): Promise<FipeBrand[]> {
  ...
  const cached = readCache<FipeBrand[]>(cacheKey);
