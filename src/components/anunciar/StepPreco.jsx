...
export default function StepPreco({ dados, setDados, onBack, onPublicar }) {
  ...
  const validar = () => {
    const novosErros = {};
    if (!dados.preco || dados.preco <= 0) novosErros.preco = true;
  ...
  const getSugestaoPreco = (valor) => {
    if (!valor || valor <= 0) return 'Pode anunciar carros de qualquer valor (baixo custo ou premium).';
    if (valor <= 2000) return '💡 Preço low-cost! Ótimo para atrair compradores rápidos.';
    if (valor <= 10000) return '💰 Preço médio. Considere destacar o bom estado do veículo.';
